...
class MessageService {
  send(
    ...
    return this.create(
      ...
      id_project,
      createdBy,
      MessageConstants.CHAT_MESSAGE_STATUS.SENDING,
      attributes,
      type,
      metadata,
      language
  ...
  save(message) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      messagePromiseEvent
        .emit("message.create.simple.before", { beforeMessage: beforeMessage })
        .then((results) => {
          winston.debug("message.create.simple.before results", results);
          winston.debug(
            "message.create.simple.before results prototype: " +
              Object.prototype.toString.call(results)
          );

          if (results) {
            winston.debug(
              "message.create.simple.before results.length: " + results.length
            ); //TODO ELIMINA DOPO CHE CREA CRASH
          }
          ...
          return newMessage.save(function (err, savedMessage) {
            if (err) {
              winston.error("Error saving the message", {
                err: err,
                message: message,
  ...
  getTranscriptByRequestId(requestid, id_project) {
    ...
    var that = this;
    return new Promise(function (resolve, reject) {
      return Message.find({ recipient: requestid, id_project: id_project })
        .sort({ createdAt: "asc" })
