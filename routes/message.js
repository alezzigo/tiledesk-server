...
router.post(
  ...
  async (req, res) => {
    ...
    let messageStatus =
      req.body.status || MessageConstants.CHAT_MESSAGE_STATUS.SENDING;
    ...
    return q.exec(async (err, request) => {
      ...
      if (err) {
        ...
        return res.status(500).send({
          success: false,
          msg: "Error getting the request.",
          err: err,
        });
      }

      if (!request) {
        //the request doen't exists create it
        ...
        if (sender) {
          ...
          project_user = await Project_user.findOne(queryProjectUser).populate({
            path: "id_user",
          ...
          if (!project_user) {
            return res.status(403).send({
              success: false,
              msg:
                "Unauthorized. Project_user not found with user id  : " +
                sender,
            });
          }
          ...
          if (project_user.id_user) {
            ...
            var lead = await Lead.findOne({
              lead_id: project_user.uuid_user,
              id_project: req.projectid,
            });
        ...
        return leadService
          .createIfNotExistsWithLeadId(
            sender || req.user._id,
            fullname,
            email,
            req.projectid,
            null,
            req.body.attributes || req.user.attributes
          )
                  ...
                  .then(function (savedMessage) {
                    ...
                    savedRequest //bug
                      ...
                      .execPopulate(function (err, savedRequestPopulated) {
                        //bug with  execPopulate request.attributes are invalid (NOT real data). but this bug is related to chat21 listener changes by reference. i think populate suffer from this problem bacause it it the same obect passed by reference

                        if (err) {
                          return winston.error(
                            "Error gettting savedRequestPopulated for send Message",
                            err
                          );
                        }

                        winston.debug(
                          "returning savedRequest221 to",
                          savedRequest.toJSON()
                        );

                        winston.debug(
                          "savedRequestPopulated",
                          savedRequestPopulated.toJSON()
                        );

                        winston.debug(
                          "returning savedRequest22 to",
                          savedRequest.toJSON()
                        );
        ...
        return messageService
          ...
          .then(function (savedMessage) {
            ...
            if (
              ...
              request.participants.indexOf(sender) > -1
            ) {
              //update waiitng time if write an  agent (member of participants)
              winston.debug("updateWaitingTimeByRequestId");
              return requestService
                .updateWaitingTimeByRequestId(
                  request.request_id,
                  request.id_project,
                  true
                )
                .then(function (upRequest) {
                  let message = savedMessage.toJSON();
          ...
          .catch(function (err) {
            winston.log({
              level: "error",
              message:
                "Error creating message endpoint: " +
                JSON.stringify(err) +
                " " +
                JSON.stringify(req.body),
              label: req.projectid,
...
router.post("/multi", async (req, res) => {
  ...
  return q.exec(async (err, request) => {
    if (err) {
      ...
      return res
        .status(500)
        .send({ success: false, msg: "Error getting the request.", err: err });
    }
    ...
    if (!request) {
      ...
      return res
        .status(404)
        .send({ success: false, msg: "Request doesn't exist.", err: err });
    } else {
      winston.debug("request  exists", request.toObject());
      ...
      req.body.forEach(function (message, index) {
        ...
        // create(sender, senderFullname, recipient, text, id_project, createdBy, status, attributes, type, metadata, language, channel_type, channel) {
        let promise = messageService.create(
          sender || req.user._id,
          fullname,
          req.params.request_id,
          text,
      ...
      PromiseUtil.doAllSequentually(promises)
        .then((values) => {
        ...
        .catch((err) => {
          ...
          return res.status(500).send({
            success: false,
            msg: "Error inserting multiple messages.",
            err: err,
          });
        });
    }
...
router.get("/csv", function (req, res) {
  ...
  return Message.find({
    recipient: req.params.request_id,
    id_project: req.projectid,
  })
    .sort({ createdAt: "asc" })
...
router.get("/:messageid", function (req, res) {
  ...
  Message.findById(req.params.messageid, function (err, message) {
    if (err) {
      return res
        .status(500)
        .send({ success: false, msg: "Error getting object." });
    }
    if (!message) {
