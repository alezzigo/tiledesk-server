...
router.get('/user-input-results', function (req, res, next) {
  ...
  // query.participantsBots = ['6646b9540c626e0013c0dc14'];
  ...
  Promise.all(promises).then(function ([requestRes, countRes]) {
    const getParametersPromises = requestRes.map(o => {
      return requestService.getRequestParametersFromChatbot(o.request_id)
    })

    Promise.all(getParametersPromises).then(function (results) {
      var objectToReturn = {
        perPage: limit,
        count: countRes,
        requests: results.map((r, index) =>
        ({
          ...requestRes[index].toObject({ getters: false }),
          userInput: r
        })
        )
      };
